...
export default function EventsCard({ item }: { item: EventData }) {
    ...
    return (
        <div className="bg-white rounded-xl shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] hover:shadow-xl transition-shadow duration-300 border border-gray-100 overflow-hidden group flex flex-col md:flex-row h-auto md:h-64">
            ...
            {/* Image Section */}
            <div className="relative w-full md:w-[40%] h-40 md:h-full bg-gray-100 overflow-hidden cursor-pointer">
            ...
            {/* Content Section */}
            <div className="flex-1 p-2.5 md:p-5 flex flex-col justify-between">
                <div>
                    <h3 className="font-bold text-lg text-gray-800 leading-tight mb-0.5 line-clamp-1 hover:text-[#FF6E40] transition-colors cursor-pointer">
                ...
                <div className="flex items-center justify-between pt-2 border-t border-gray-50">
                    ...
                    <div className="flex gap-2">
                        <Link href={`/events/${item.id}`} className="flex items-center gap-2 px-3 py-2 rounded bg-[#FFF0EB] text-[#FF6E40] text-xs font-bold hover:bg-[#FF6E40] hover:text-white transition-all shadow-sm border border-[#FFCCBC]/50">
